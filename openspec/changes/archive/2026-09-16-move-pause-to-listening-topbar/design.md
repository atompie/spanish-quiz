...
- **Icon-only button, state-dependent style**: the pause/resume button uses the existing `.btn-icon` class, adding the existing `--inverted` accent modifier (already used by `TopBarCloseButton` and the active `answer-wait-picker` option) only while paused, so the icon reads as "selected" when the session is paused, and plain otherwise.
